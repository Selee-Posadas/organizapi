...
export class EvaluationController {
    ...
    async findByType(@Param('type') type: EvaluationType, @GetUser('id') userId: string) {
        return await this.findByTypeUseCase.execute(type, userId);
    }

    @Get('search/status/:status')
    async findByStatus(@Param('status') status: string, @GetUser('id') userId: string) {
        return await this.findByStatusUseCase.execute(status, userId);
    }

    @Get('search/day')
    async findByDay(@Query('date') dateStr: string, @GetUser('id') userId: string) {
