...
export class ScheduleController {
    ...
    @Get('search/day/:day')
    async findByDay(@Param('day', ParseIntPipe) day: DayOfWeek, @GetUser('id') userId: string) {
        return await this.findByDayUseCase.execute(day, userId);
    }

    @Get('search/type/:type')
    async findByType(@Param('type') type: ScheduleType, @GetUser('id') userId: string) {
        return await this.findByTypeUseCase.execute(type, userId);
    }

    @Get('search/location')
    async findByLocation(@Query('location') location: string, @GetUser('id') userId: string) {
        return await this.findByLocationUseCase.execute(location, userId);
    }

    @Get('search/time')
    async findByStartTime(@Query('start') startTime: string, @GetUser('id') userId: string) {
        return await this.findByStartTimeUseCase.execute(startTime, userId);
    }

    @Get('enrollment/:enrollmentId')
    async findByEnroll(@Param('enrollmentId', ParseUUIDPipe) enrollmentId: string, @GetUser('id') userId: string) {
    ...
    @Patch(':id')
    async update(@Param('id', ParseUUIDPipe) id: string, @Body() dto: UpdateScheduleDto, @GetUser('id') userId: string) {
