...
import type { CorrelativeResult, SubjectRepository } from "src/university/domain/repositories/subject.repository";
...
export class GetCorrelativesUseCase {
    ...
    async execute(subjectId: string, userId: string): Promise<CorrelativeResult[]> {
        if (!subjectId) {
