...
export class RemoveCorrelativeUseCase {
    ...
    async execute(subjectId: string, requiredId: string, userId: string): Promise<void> {
        ...
        const subject = await this.subjectRepository.findSubjectById(subjectId, userId);
        if (!subject) throw new NotFoundException('Subject not found');

        await this.subjectRepository.removeCorrelative(subjectId, requiredId, userId);
    }
}