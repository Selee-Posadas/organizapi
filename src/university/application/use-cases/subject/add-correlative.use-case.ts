  ...
  async execute(
    ...
    if (subjectId === requiredId) {
      throw new BadRequestException(
        'A subject cannot be correlative of itself',
      );
    }
    ...
    if (!allowedStatuses.includes(type)) {
      throw new BadRequestException(
        'A correlative requirement must be either REGULAR or APPROVED',
      );
    }

    const validatedType = type as
      | EnrollmentStatus.REGULAR
      | EnrollmentStatus.APPROVED;

    await this.subjectRepository.addCorrelative(
      subjectId,
      requiredId,
      validatedType,
      userId,
