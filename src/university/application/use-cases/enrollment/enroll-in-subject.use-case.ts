...
export class EnrollInSubjectUseCase {
    ...
    async execute(dto: CreateEnrollmentDto, userId: string): Promise<Enrollment> {
        ...
        const existingEnrollment = await this.enrollRepository.findEnrollmentBySubject(dto.subjectId, userId);
        ...
        const enrollmentMap = new Map(allUserEnrollments.map(e => [e.subjectId, e]));
            ...
            for (const req of requirements) {
                ...
                if (req.type === EnrollmentStatus.APPROVED) {
                    if (!priorEnrollment || priorEnrollment.status !== EnrollmentStatus.APPROVED) {
                        throw new BadRequestException(`Prerequisite mismatch: Subject '${req.requiredSubject.name}' must be APPROVED before enroll in this subject..`);
                    }
                }

                if (req.type === EnrollmentStatus.REGULAR) {
                    const isRegular = priorEnrollment && [
                    ...
                    if (!isRegular) {
                        throw new BadRequestException(`Academic requirement not met: You need at least REGULAR status in '${req.requiredSubject.name}' to proceed.`);
                    }
                }
            }
        }
